async function loadWeb3() {
  ...
  async function loadContract() {
    return await new window.web3.eth.Contract(
      ...
      '0xa2F905e7Fd177551d7A0a03Fcd3F9360cd9AD850',
    )
  }
  async function startMint(){
    mint(await getCurrentAccount(), getText())
  }
  async function getCount() {
    updateStatus('fetching Number...')
    console.log(window.contract.methods)
    const number = await window.contract.methods.getCount().call()
    updateStatus(`coolNumber: ${number}`)
  }

  async function incrementCounter() {
    updateStatus(`Increasing...`)
  ...
  async function getText() {
    let msg = document.querySelector('#thought').value
    return msg
  }

  async function mint(to, str) {
